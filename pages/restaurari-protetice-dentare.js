...
const ImplantologieRestaurariProtetice = () => {
  const [indexCaz, setIndexCaz] = useState(0);
  const [imgNo, setImgNo] = useState(0);
  const router = useRouter();
  const serviceId = 1;
  const procedureIndex = 1;
    ...
    <div className='services-container m-auto'>
      <div className='pt-4 pb-4 bg-gray' >
        <div className='m-auto' style={{ maxWidth: '60rem' }}>
          <IconLink label={`Înapoi la ${services[serviceId]?.title}`} href={`/implantologie`}>
...
export default ImplantologieRestaurariProtetice;